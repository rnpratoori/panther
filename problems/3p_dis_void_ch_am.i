...
Cn_eta = 2e-2  # Cahn number for AC variable
...
[Materials]
    ...
    [mobility_eta]
        ...
        expression = 'if(c3>0.1, M*1e6*(c3-0.1), 0)'
        # block = 0
    []
    [mobility1]
        type = DerivativeParsedMaterial
    ...
    [beta_penalty]
        ...
        expression = 'if(c1>1e-2, if(c2>1e-2, if(1-c1-c2>1e-2, beta*(1/c1 + 1/c2 + 1/(1 - c1 - c2)), beta*(1/c1 + 1/c2)), if(1-c1-c2>1e-2, beta*(1/c1 + 1/(1 - c1 - c2)), 0)), if(c2>1e-2, if(1-c1-c2>1e-2, beta*(1/c2 + 1/(1 - c1 - c2)), beta*(1/c2)), if(1-c1-c2>1e-2, beta*(1/(1 - c1 - c2)), 0)))'
        derivative_order = 2
        # block = 0
    []
    # Total free energy
...
[Outputs]
    [ex]
        type = Exodus
        file_base = output/3p_dis_void_ch_am_4
