...
# Automatically generate the array_range based on the number of values for a, x, and N
num_a=${#a_values[@]}  # Number of values for a
num_x=${#x_values[@]}  # Number of values for x
num_N=${#N_values[@]}  # Number of values for N

array_range=$((num_a * num_x * num_N))

# Loop over all combinations
for idx in $(seq 0 $((array_range - 1))); do
  # Calculate the index for a, x, and N
  a_idx=$((idx / (num_x * num_N)))       # Index for a
  x_idx=$(((idx / num_N) % num_x))       # Index for x
  N_idx=$((idx % num_N))                 # Index for N

  # Get the actual values for a, x, and N
  a=${a_values[$a_idx]}
  x=${x_values[$x_idx]}
  N=${N_values[$N_idx]}

  echo "Running simulation for: a=$a, x=$x, N=$N"

  # Create a unique directory for this combination
  task_dir="$output_dir/DBC_$((idx+1))_a${a}_x${x}_N${N}"
  mkdir -p "$task_dir"
  cd "$task_dir" || exit 1
  ...
  log_file="$task_dir/output_${idx}_a${a}_x${x}_N${N}.log"
  err_file="$task_dir/error_${idx}_a${a}_x${x}_N${N}.log"
