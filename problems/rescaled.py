...
if __name__ == '__main__':
    source_file = "output/2phase_fh.e"
    copy_file = "output/2phase_copy_scaled.e"
    delta = 0.01  # Change this value as needed
    ...
    scaled_values = source_values * (1 - delta)
    print(f"Scaled c range: [{np.min(scaled_values):.6f}, {np.max(scaled_values):.6f}]")

    overwrite_nodal_variable(copy_file, 'c', 'c1_scaled', scaled_values)

    # # Also scale 1-c if you want a complement variable
    # complement_values = 1 - scaled_values
    # print(f"Complement (1-c1_scaled) range: [{np.min(complement_values):.6f}, {np.max(complement_values):.6f}]")

    # overwrite_nodal_variable(copy_file, 'c2', 'c2_scaled', complement_values)

    # Scale c2 by (1 - delta)
    c2_values = get_values(source_file, 'c2')   
    c2_scaled = c2_values * (1 - delta)
    print(f"Scaled c2 range: [{np.min(c2_scaled):.6f}, {np.max(c2_scaled):.6f}]")

    overwrite_nodal_variable(copy_file, 'c2', 'c2_scaled', c2_scaled)

    print(f"\nCreated {os.path.basename(copy_file)} with modified variables 'c_scaled' and 'c2_scaled'")
